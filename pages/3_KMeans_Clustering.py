...
# # Hanya jalankan elbow_method jika belum pernah disimpan
#     if 'pivot' not in st.session_state or 'distortions' not in st.session_state:
#         with st.spinner("Menghitung inertia untuk berbagai jumlah cluster..."):
#             pivot, distortions = elbow_method(df)
#             st.session_state.pivot = pivot
...
#     # Tampilkan Elbow Chart
#     st.subheader("📉 Elbow Method")
#     show_elbow_chart(distortions)

#     # # Slider untuk memilih jumlah cluster (default ke yang terakhir dipilih, atau 3)
#     default_k = st.session_state.get("selected_k", 3)
#     # k = st.slider("Pilih Jumlah Cluster", min_value=2, max_value=10, value=default_k, step=1)
#     k = st.selectbox("Jumlah Cluster", options=[3], index=0)

#     # Tombol untuk menjalankan clustering
#     if st.button("🔄 Jalankan Clustering"):
#         with st.spinner("Melakukan clustering..."):
#             clustered = run_kmeans(df, n_clusters=k)
#             st.session_state.clustered_df = clustered
#             st.session_state.selected_k = k  # simpan k terakhir yang digunakan

#         st.success("✅ Clustering selesai!")

#     # Tampilkan hasil clustering terakhir jika sudah ada
#     if 'clustered_df' in st.session_state:
#         clustered = st.session_state.clustered_df
...
#         st.subheader("📦 Rincian Tiap Cluster")
#         for i in sorted(clustered['cluster'].unique()):
#             st.markdown(f"### Cluster {i} - {clustered[clustered['cluster'] == i]['kategori'].iloc[0]}")
#             st.dataframe(clustered[clustered['cluster'] == i])
